...
alldata = spark.read.format("csv").option("header", True).option("delimiter",";").load("hdfs://localhost:9000/data_upload/master/JD+Shopee+Lazada.csv")
df = alldata.toPandas()

df.columns
df = df.drop(['enToken'],axis=1)
for index, row in df.iterrows():
    row['thToken'] = word_tokenize(row['Category'])
token =[]
for index, row in df.iterrows():
    token.append(row['thToken'])
...
df_copy_copy.Category.drop_duplicates()
df_copy_copy=df_copy_copy.rename(columns={"thToken": "Token"})
for index, row in df_copy_copy.iterrows():
...
result.to_csv('./result/result123.csv',encoding='utf-8-sig',sep=';',index= False)
